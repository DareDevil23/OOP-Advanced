namespace TheTankGame.Core
{
    ...
    public class TankManager : IManager
    {
        ...
        public string AddPart(IList<string> arguments)
        {
            ...
            var methodName = $"Add{partType}Part";
            this.vehicles[vehicleModel].GetType().GetMethod(methodName)
                .Invoke(this.vehicles[vehicleModel], new object[] {part});

            return string.Format(
                GlobalConstants.PartSuccessMessage, partType, part.Model, vehicleModel);
        ...
        public string Inspect(IList<string> arguments)
        {
            ...
            string result = this.vehicles.ContainsKey(model) ? this.vehicles[model].ToString() : this.parts[model].ToString();
        ...
        public string Terminate(IList<string> arguments)
        {
            ...
            finalResult.AppendLine(this.vehicles.Count > 0 ? string.Join(", ", this.vehicles.Values.Select(v => v.Model).ToArray()) : "None");

            finalResult.Append("Defeated Vehicles: ");

            finalResult.AppendLine(this.defeatedVehicles.Count > 0 ? string.Join(", ", this.defeatedVehicles) : "None");

            finalResult.Append("Currently Used Parts: ").Append(this.parts.Count);
