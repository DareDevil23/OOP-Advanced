	...
	public class FlightController : IFlightController
	{
		...
		public string TakeOff()
		{
			...
			foreach (var trip in this.airport.Trips)
			{
				...
				var tripResult =
				    $"Successfully transported {trip.Airplane.Passengers.Count} passengers from {trip.Source} to {trip.Destination}.";
			...
			sb.AppendLine(
			    $"Confiscated bags: {confiscatedBags.Count} ({confiscatedItems.Length} items) => ${confiscatedItemsSum}");
